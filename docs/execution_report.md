...
1. **Gestão do "Memory Wall":** Com as diretivas SIMD e o `aligned_alloc` aplicados no código em C, os núcleos da CPU processam blocos de 64-bytes de forma quase instantânea. Quando forçamos o *pinning* estático em 4 núcleos físicos distintos, todas as *threads* colidem no controlador de memória simultaneamente exigindo novos blocos da RAM. O escalonamento dinâmico do SO introduz micro-atrasos e flutuações naturais que acabam dessincronizando levemente as *threads*, aliviando o tráfego no barramento de memória (Memory Bus).
...
A implementação atingiu o seu objetivo com sucesso. A combinação de otimizações sequenciais profundas (matrizes em layout 1D, `aligned_alloc`, remoção de ramificações no cálculo de Mínimo/Máximo) e paralelizáveis (Welford *thread-safe* de passagem única, buffers locais no *stack* em vez de disputas no *heap*) gerou uma base sólida. 
